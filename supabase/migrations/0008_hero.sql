-- 0008 — Per-entity hero image. Stored as a small jsonb pointer on entities:
--   {"url":"heroes/<id>.jpg","kind":"enhanced"|"generated","source":"<og:image url|null>"}
-- 'enhanced' = the org's own photo run through an image model; 'generated' = no
-- usable photo found, a representative image was created (badged "AI Generated").

alter table entities add column if not exists hero jsonb;

-- Rebuild entity_full so e.* picks up the hero column.
